...
class CrossFrequencyScale(nn.Module):
    ...
    def forward(self, x_a, x_b):
        B, C, H, W, = x_a.shape
        xa_rfft = torch.fft.rfft2(x_a.to(torch.float32), dim=(2, 3), norm='ortho')
        B, C, RH, RW, = xa_rfft.shape
        x_b = x_b.permute(0, 2, 3, 1)

        routeing = self.reweight(x_b.mean(dim=(1, 2))).view(B, -1, self.num_filters).tanh_() # b, num_filters, group
        weight = self.complex_weights
        if not weight.shape[2:4] == xa_rfft.shape[2:4]:
            weight = F.interpolate(weight, size=xa_rfft.shape[2:4], mode='bicubic', align_corners=True)

        weight = torch.einsum('bgf,fchw->bgchw', routeing, weight)
        weight = weight.reshape(B, C, RH, RW)

        xa_rfft = torch.view_as_complex(torch.stack([xa_rfft.real * weight, xa_rfft.imag * weight], dim=-1))
        x_a = torch.fft.irfft2(xa_rfft, s=(H, W), dim=(2, 3), norm='ortho')
        return x_a
