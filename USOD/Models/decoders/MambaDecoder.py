...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        if self.deep_supervision:
            ...
            self.output_dd = nn.ModuleList([nn.Conv2d(in_channels=embed_dim * 2 ** (self.num_layers - 2 - i_layer), out_channels=self.num_classes, kernel_size=1, bias=False) for i_layer in range(self.num_layers-1)])
    ...
    def forward(self, inputs):
        if not self.deep_supervision:
            ...
            x_last = self.up_x4(x, self.patch_size) # 16 -> 224
            x_output_0 = self.output_ds[0](F.interpolate(x_upsample[0].permute(0,3,1,2).contiguous(), scale_factor=16, mode='bilinear', align_corners=False)) #14 -> 224
            x_output_1  = self.output_ds[1](F.interpolate(x_upsample[1].permute(0,3,1,2).contiguous(), scale_factor=8, mode='bilinear', align_corners=False)) #28 -> 224
            x_output_2 = self.output_ds[2](F.interpolate(x_upsample[2].permute(0,3,1,2).contiguous(), scale_factor=4, mode='bilinear', align_corners=False)) #56 -> 224
            return x_last, x_output_0, x_output_1, x_output_2
            '''
            x, x_upsample = self.forward_up_features(inputs)

            x4 = self.output_ds[0](x_upsample[0].permute(0,3,1,2).contiguous())
            ux4 = self.output_dd[0](F.interpolate(x_upsample[0].permute(0,3,1,2).contiguous(), scale_factor=16, mode='bilinear', align_corners=False))

            x3 = self.output_ds[1](x_upsample[1].permute(0,3,1,2).contiguous())
            ux3 = self.output_dd[1](F.interpolate(x_upsample[1].permute(0,3,1,2).contiguous(), scale_factor=8, mode='bilinear', align_corners=False))

            x2 = self.output_ds[2](x_upsample[2].permute(0,3,1,2).contiguous())
            ux2 = self.output_dd[2](F.interpolate(x_upsample[2].permute(0,3,1,2).contiguous(), scale_factor=4, mode='bilinear', align_corners=False))

            x_last = self.up_x4(x, self.patch_size) # 16 -> 224

            return F.sigmoid(x_last), F.sigmoid(x2), F.sigmoid(x3), F.sigmoid(x4), F.sigmoid(ux2), F.sigmoid(ux3), F.sigmoid(ux4)
