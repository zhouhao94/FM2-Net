...
class ImageDepthNet(nn.Module):
    ...
    def forward(self, image_Input, depth_Input):
        ...
        feature_map1 = self.rgb_backbone(image_Input, layer_flag=1) # feature_map1: B x 3 x 224 x 224
        dep_layer3_vit, _, _, dep_layer1, dep_layer2, dep_layer3 = self.depth_backbone(depth_Input) # dep_layer3_vit: B x 196 x 384
        #print("1", feature_map1.shape, dep_layer3_vit.shape)

        img_cmf1 = self.DAM1(feature_map1, dep_layer1)
        img_layer_cat1 = feature_map1 + img_cmf1
        feature_map2, rgb_fea_1_4 = self.rgb_backbone(img_layer_cat1, layer_flag=2) # feature_map2: B x 64 x 56 x 56, rgb_fea_1_4: B x 3136 x 56 x 56
        #print("2", feature_map2.shape, rgb_fea_1_4.shape)

        img_cmf2 = self.DAM2(feature_map2, dep_layer2)
        img_layer_cat2 = feature_map2 + img_cmf2
        feature_map3, rgb_fea_1_8 = self.rgb_backbone(img_layer_cat2, layer_flag=3) # feature_map3: B x 64 x 28 x 28, rgb_fea_1_8: B x 784 x 64
        #print("3", feature_map3.shape, rgb_fea_1_8.shape)

        img_cmf3 = self.DAM3(feature_map3, dep_layer3)
        img_layer_cat3 = feature_map3 + img_cmf3

        img_layer3_vit = self.rgb_backbone(img_layer_cat3, image_Input, layer_flag=4) # img_layer3_vit: B x 196 x 384
        #print("4", img_layer3_vit.shape)

        rgb_fea_1_16, depth_fea_1_16 = self.transformer(img_layer3_vit, dep_layer3_vit) # rgb_fea_1_16: B x 196 x 384, depth_fea_1_16: B x 196 x 384
        #print("5", rgb_fea_1_16.shape, depth_fea_1_16.shape)
